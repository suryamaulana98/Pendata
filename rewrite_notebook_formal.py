  ...
  "cells": [
    {
      ...
      "source": [
        ...
        "1. **Analisis Visual** menggunakan representasi grafis dengan **GeoGebra**.\n",
        "2. **Analisis Matematis / Analitik** menerapkan model matriks berbasis metode Kuadrat Terkecil Biasa (*Ordinary Least Squares* OLS) dengan rumus $\\hat{\\beta} = (X^T X)^{-1} X^T Y$.\n",
        "3. **Analisis Komputasional** menggunakan pustaka standard *Machine Learning* dalam ekosistem Python, yaitu `scikit-learn`."
      ]
    },
    {
      "cell_type": "markdown",
      ...
      "source": [
        ...
        "\n",
        "> **📸 [INSTRUKSI: TEMPATKAN GAMBAR PERTAMA DI SINI]**\n",
        "> \n",
        "> *Sisipkan tangkapan layar (screenshot) luasan grafik berupa 'scatter plot' dari antarmuka GeoGebra yang memuat titik-titik observasi (A hingga G) yang belum diberikan garis.* \n",
        "> *Format Markdown: `![Diagram Pencar GeoGebra](images/geogebra_scatter.png)`*\n",
        "\n",
        "**Interpretasi Grafis:**\n",
        "Berdasarkan visualisasi diagram pencar matriks koordinat tersebut, distribusi sekelompok observasi membentuk sebuah tren yang cenderung bergerak ekuivalen naik dari arah kuadran kiri bawah menuju ke kanan atas. Formasi ini mengindikasikan eksistensi **korelasi linier positif**, di mana sebuah kenaikan pada skala nilai $X$ cenderung akan diikuti pula oleh kenaikan proporsional terhadap estimasi nilai $Y$.\n",
        "\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "1. Mendesain kerangka **Matriks X**. Kolom pertama diisi dengan nilai observasi konstan 1 (merepresentasikan titik intersepsi), sedangkan sisanya merangkum semua susunan nilai prediktor variabel $X$.\n",
        "2. Mendesain kerangka **Vektor Y**, bermuatan observasi empiris titik parameter akhir.\n",
        "3. Memproduksi hasil transposisi Matriks X yang dilambangkan dengan $X^T$.\n",
        "4. Menjalankan operasi ekuasi turunan perkalian skalar (*dot-product*) antara format matriks ter-transpos, $X^T$, dan parameter matriks orisinal, $X$, sehingga membentuk model matrik gramian: $(X^T X)$.\n",
        "5. Mengkonversi inversi balikan operasi tersebut ke derivatif: $(X^T X)^{-1}$.\n",
        "6. Perkalian berlanjut terhadap elemen $X^T$ dan akhirnya dikomputasikan silang terhadap parameter vektor target $Y$.\n",
        "7. Parameter nilai $\\hat{\\beta}$ dari formulasi di atas merepresentasikan struktur hierarkis nilai target dari elemen **Intercept** maupun **Slope** pada model data matematis.\n",
        "\n",
    ...
    },
    {
      ...
      "source": [
        ...
        "**`y = 1.0769x`**\n",
        "\n",
        "> **📸 [INSTRUKSI: TEMPATKAN GAMBAR KEDUA DI SINI]**\n",
        "> \n",
        "> *Arahkan pengalokasian fungsi grafis pada bilah instrumen GeoGebra dengan pengetikan argumen kalkulus: `FitLine(A, B, C, D, E, F, G)`. Integrasi sistem akan merekonstruksi garis proyeksi lintasan. Tangkap penulisan sintaks beserta grafiknya lalu implementasikan format referensinya di sini.*\n",
        "> *Format referensi Markdown misalnya:* `![Garis Regresi Komputasi GeoGebra](images/geogebra_regresi_garis.png)`\n",
        "\n",
        "**Signifikansi Pemodelan**\n",
        "\n",
        "Formasi fungsi garis estimator linier yang dirender secara simetris dalam GeoGebra, dan disusul integrasi kalkulasi di lingkup algoritma Python, memastikan keberhasilan mereduksi total distorsi error residu (*residual loss*). Pencapaian kalkulasi ini merepresentasikan substansi fungsional terpenting bagi permodelan _Data Science_, yaitu kapabilitas prediksi data ekstrapolasi empiris luar model.\n",
        "\n",
...
with open("materi/Analisa_Data_Regresi_Linierx.ipynb", "w", encoding="utf-8") as f:
    json.dump(notebook_content, f, indent=2, ensure_ascii=False)

print("Formal rewrite complete")
